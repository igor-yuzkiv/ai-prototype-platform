...
export const projectArtifactsApi = {
    async list(params?: PaginationParams): Promise<PaginatedCollectionResponse<ProjectArtifact>> {
        const response = await httpClient.get<PaginatedCollectionResponse<ProjectArtifact>>('/project-artifacts', {
            params,
        })
    ...
    async create(payload: CreateProjectArtifactPayload): Promise<ProjectArtifact> {
        const response = await httpClient.post<ResourceResponse<ProjectArtifact>>('/project-artifacts', payload)

        return unwrapResource(response.data)
    },

    async update(projectArtifactId: number | string, payload: UpdateProjectArtifactPayload): Promise<ProjectArtifact> {
